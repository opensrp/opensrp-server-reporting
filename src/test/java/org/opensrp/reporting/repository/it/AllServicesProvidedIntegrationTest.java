...
public class AllServicesProvidedIntegrationTest extends ServicesProvidedIntegrationTestBase {
	...
	public void shouldSaveAService() throws Exception {
		...
		ServiceProviderType anmServiceProvider = selectUnique(serviceProviderTypes,
		    having(on(ServiceProviderType.class).type(), equalTo(ANM.type())));
		...
		assertTrue(servicesProvided
		        .contains(new ServiceProvided(serviceProvider, "123", indicator, dates, location, dristhiEntityId)));
	}
	...
	public void shouldUpdateIndicatorForReportingMonth() throws Exception {
		...
		List<ServiceProvided> servicesProvided = template.loadAll(ServiceProvided.class);
		assertFalse(servicesProvided.contains(
	...
	public void shouldFetchAllReportsWhenTokenIsZero() throws Exception {
		...
		assertTrue(reports.contains(new ServiceProvidedReport(1, "ANM X", ANM.type(), "INDICATOR",
		        LocalDate.parse("2013-01-26").toDate(), "Bherya", "Sub Center", "bhe", "taluka", "mysore", "karnataka")));
	}
	...
	public void shouldNotFetchNewReportsWhenTokenIsEqualToMostRecentReport() throws Exception {
		...
		assertFalse(reports.contains(new ServiceProvidedReport(1, "ANM X", ANM.type(), "INDICATOR",
		        LocalDate.parse("2013-01-26").toDate(), "Bherya", "Sub Center", "bhe", "taluka", "mysore", "karnataka")));
	}
