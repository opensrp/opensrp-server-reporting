...
public class AllAnnualTargetsRepositoryIntegrationTest extends ANMReportsIntegrationTestBase {
	...
	public void shouldFetchAnnualTargetForGivenANMAndIndicator() throws Exception {
		...
		template.save(anm);
		template.save(
		    new AnnualTarget(anm.id(), indicator.id(), "40", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
	...
	public void shouldFetchAnnualTargetIfReportDateIsStartOrEndOfRange() throws Exception {
		...
		template.save(anm);
		template.save(new AnnualTarget(anm.id(), firstIndicator.id(), "40", parse("2012-03-26").toDate(),
		        parse("2013-03-25").toDate()));
		template.save(new AnnualTarget(anm.id(), secondIndicator.id(), "80", parse("2012-03-26").toDate(),
		        parse("2013-03-25").toDate()));
		
		AnnualTarget annualTargetForFirstIndicator = repository.fetchFor(anm.anmIdentifier(), firstIndicator,
		    parse("2012-03-26").toDate());
		AnnualTarget annualTargetForSecondIndicator = repository.fetchFor(anm.anmIdentifier(), secondIndicator,
		    parse("2013-03-25").toDate());
