...
import static org.opensrp.common.util.ANMIndicatorSummaryFactory.createSummaryForCondom;
...
public class ANMReportsRepositoryTest {
	...
	public void setUp() throws Exception {
		initMocks(this);
		repository = new ANMReportsRepository(anmRepository, indicatorRepository, anmReportDataRepository,
		        annualTargetsRepository, monitor, new ReportMonth());
	}
	...
	public void shouldFetchANMIndicatorSummaries() throws Exception {
		...
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate())).thenReturn(
		    asList(new ANMReportData(anm, "CASE 1", indicator1, date1), new ANMReportData(anm, "CASE 2", indicator1, date1),
		        new ANMReportData(anm, "CASE 3", indicator1, date2), new ANMReportData(anm, "CASE 4", indicator1, date2),
		        new ANMReportData(anm, "CASE 5", indicator2, date3), new ANMReportData(anm, "CASE 6", indicator2, date3)));
		when(annualTargetsRepository.fetchFor("ANM X", indicator1, parse("2012-06-30").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "40", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
		when(annualTargetsRepository.fetchFor("ANM X", indicator2, parse("2012-06-30").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "30", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
	...
	public void shouldSetMonthOfSummaryToBeNextYearsInCaseOfLastMonthOfTheYear() throws Exception {
		...
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate()))
		        .thenReturn(asList(new ANMReportData(anm, "CASE 1", indicator, date)));
		when(annualTargetsRepository.fetchFor("ANM X", indicator, parse("2013-01-30").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "40", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
		
		List<ANMIndicatorSummary> anmIndicatorSummaries = repository.fetchANMSummary("ANM X");
		
		assertEquals(
		    asList(new ANMIndicatorSummary("IUD", "40", asList(new MonthSummary("1", "2013", "1", "1", asList("CASE 1"))))),
		    anmIndicatorSummaries);
	}
	...
	public void shouldFetchANMIndicatorSummariesIncludingAllServicesProvidedTillTheCurrentMonth() throws Exception {
		...
		Indicator indicator1 = new Indicator("IUD");
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate()))
		        .thenReturn(asList(new ANMReportData(anm, "CASE 1", indicator1, january)));
		when(annualTargetsRepository.fetchFor("ANM X", indicator1, parse("2013-01-15").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "40", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
	...
	public void shouldFetchANMIndicatorSummariesWithDistinctExternalIds() throws Exception {
		...
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate())).thenReturn(
		    asList(new ANMReportData(anm, "CASE 7", indicator, dates), new ANMReportData(anm, "CASE 7", indicator, dates)));
		when(annualTargetsRepository.fetchFor("ANM X", indicator, parse("2013-03-22").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "20", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
	...
	public void shouldReturnANMIndicatorSummariesWithNullAnnualTargetWhenThereIsNoAnnualTargetForANM() throws Exception {
		...
		Indicator indicator = new Indicator("IUD");
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate())).thenReturn(
		    asList(new ANMReportData(anm, "CASE 1", indicator, date1), new ANMReportData(anm, "CASE 2", indicator, date1),
		        new ANMReportData(anm, "CASE 3", indicator, date2), new ANMReportData(anm, "CASE 4", indicator, date2)));
	...
	public void shouldReturnAllANMReports() throws Exception {
		...
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM X", parse("2012-03-26").toDate())).thenReturn(asList(
		    new ANMReportData(anmX, "CASE 1", indicator1, date1), new ANMReportData(anmX, "CASE 2", indicator1, date1),
		    new ANMReportData(anmX, "CASE 3", indicator1, date2), new ANMReportData(anmX, "CASE 4", indicator1, date2)));
		when(anmReportDataRepository.fetchByANMIdAndDate("ANM Y", parse("2012-03-26").toDate())).thenReturn(asList(
		    new ANMReportData(anmY, "CASE 5", indicator2, date3), new ANMReportData(anmY, "CASE 6", indicator2, date3)));
		when(annualTargetsRepository.fetchFor("ANM X", indicator1, parse("2012-06-30").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "40", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
		when(annualTargetsRepository.fetchFor("ANM Y", indicator2, parse("2012-06-30").toDate()))
		        .thenReturn(new AnnualTarget(1, 1, "30", parse("2012-03-26").toDate(), parse("2013-03-25").toDate()));
	...
	public void shouldUpdateAllIndicatorsForReportingMonthForAllANMS() {
		...
		ReportingData reportingData = new ReportingData("type", data).withQuantity("1");
		
		ReportDataUpdateRequest request = new ReportDataUpdateRequest().withType("type").withIndicator(indicator)
		        .withStartDate("2013-01-26").withEndDate("2013-02-25").withReportingData(asList(reportingData));
	...
	public void shouldDeleteANMReportsForGivenExternalId() {
		ReportDataDeleteRequest request = new ReportDataDeleteRequest().withType("type").withDristhiEntityId("entity id 1");
