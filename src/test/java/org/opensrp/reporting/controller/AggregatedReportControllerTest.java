...
public class AggregatedReportControllerTest {
	...
	public void shouldReturnAggregatedReportDataForAGivenANMMonthAndYear() {
		when(aggregateReportsService.getAggregatedReports("demo1", 12, 2013))
		        .thenReturn(asList(new AggregatorResponseDTO("OCP", 4), new AggregatorResponseDTO("OPV_1", 2)));
		when(anmService.getLocation("demo1")).thenReturn(
		    new Location("Bherya", "sub Center", new PHC(1, "PHC 1", "Bherya"), "taluka", "mysore", "karnataka"));
		
		ResponseEntity<AggregatedReportsDTO> aggregatedReports = controller.get("demo1", 12, 2013);
		
		assertEquals(new AggregatedReportsDTO(create("OCP", 4).put("OPV_1", 2).map(),
		        new LocationDTO("Sub Center", "Bherya", "taluka", "mysore", "karnataka")),
		    aggregatedReports.getBody());
