...
public class ANMControllerTest {
	...
	public void setUp() throws Exception {
		initMocks(this);
		controller = new ANMController("site-url", "http://user-details-endpoint/user-details", anmService, httpAgent,
	...
	public void shouldReturnListOfANMs() throws Exception {
		...
		when(httpAgent.get("http://user-details-endpoint/user-details?anm-id=username1"))
		        .thenReturn(new HttpResponse(true, new Gson().toJson(new UserDetail("username1", roles))));
		
		when(detailsFetcherFactory.detailsFetcher(roles)).thenReturn(anmDetailsFetcher);
		when(anmDetailsFetcher.fetchDetails("username1"))
		        .thenReturn(asList(new SP_ANM("username1", "user1 name", "Sub Center 1", 0),
		...
		LocationDTO locationDTO = new LocationDTO(location.subCenter(), location.phcName(), location.taluka(),
		        location.district(), location.state());
		assertEquals(
		    asList(new ANMDTO("username1", "user1 name", locationDTO), new ANMDTO("username2", "user2 name", locationDTO)),
		    response.getBody());
	}
}
