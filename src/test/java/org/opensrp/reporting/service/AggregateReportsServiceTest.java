...
public class AggregateReportsServiceTest {
	...
	public void setUp() throws Exception {
		initMocks(this);
		aggregateReportsService = new AggregateReportsService("bamboo.url", "bamboo.aggregated.url", batchSizeToUpdate,
		        httpAgent, tokenRepository, servicesProvidedRepository, reportMonth);
	}
	...
	public void shouldSendReportsWithNRHMReportingMonthToAggregator() throws Exception {
		...
		String reportJson = new Gson().toJson(asList(
		    new ServiceProvidedReportDTO().withDate(LocalDate.parse("2012-11-26")).withNRHMReportMonth(12)
		            .withNRHMReportYear(2012).withId(1),
		    new ServiceProvidedReportDTO().withDate(LocalDate.parse("2012-12-28")).withNRHMReportMonth(1)
		            .withNRHMReportYear(2013).withId(2)));
	...
	public void shouldThrowExceptionWhenReportsAreNotSent() throws Exception {
		...
		when(servicesProvidedRepository.getNewReports(0, batchSizeToUpdate))
		        .thenReturn(asList(firstServiceProvidedReport, secondServiceProvidedReport));
	...
	public void shouldNotAggregateReportsWhenThereIsNoNewReport() throws Exception {
		...
		when(servicesProvidedRepository.getNewReports(12345, batchSizeToUpdate))
		        .thenReturn(Collections.<ServiceProvidedReport> emptyList());
	...
	public void shouldCallAggregatorToGetAggregatedReports() throws Exception {
		when(httpAgent.get(
		    "bamboo.aggregated.url?query=%7B%22anm_identifier%22%3A+%22demo1%22%2C%22nrhm_report_year%22%3A2013%2C+%22nrhm_report_month%22%3A10%7D"))
		            .thenReturn(new HttpResponse(true,
		                    "[{\"nrhm_report_month\": 10, \"indicator\": \"OCP\", \"nrhm_report_indicator_count\": 2, \"nrhm_report_year\": 2013, \"anm_identifier\": \"demo1\"}, {\"nrhm_report_month\": 10, \"indicator\": \"OPV_1\", \"nrhm_report_indicator_count\": 1, \"nrhm_report_year\": 2013, \"anm_identifier\": \"demo1\"}]"));
